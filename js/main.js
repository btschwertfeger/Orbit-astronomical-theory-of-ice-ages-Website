...
function calendar(epsilon, VE, int = true) {
  ...
  const day = (phi) => {
      const exzAn =
        2 *
        Math.atan(
          Math.pow((1 - epsilon) / (1 + epsilon), 0.5) * Math.tan(phi / 2),
        );
    ...
    ang = (t, t0 = 0) => {
      ...
      const phi =
        2 *
        Math.atan(
          Math.pow((1 + epsilon) / (1 - epsilon), 0.5) * Math.tan(exzAn / 2),
        );
      return phi;
    };
...
function compute(ecc, obliquity, long_perh, lat, day, day_type = 1) {
  ...
  if (day_type == 1) {
    ...
    const lambda_m0 =
      ...
      (((1 / 2) * ecc + (1 / 8) * Math.pow(ecc, 3)) *
        (1 + beta) *
        Math.sin(-omega) -
        (1 / 4) * Math.pow(ecc, 2) * (1 / 2 + beta) * Math.sin(-2 * omega) +
        (1 / 8) * Math.pow(ecc, 3) * (1 / 3 + beta) * Math.sin(-3 * omega));
    const lambda_m = lambda_m0 + delta_lambda_m;
    lambda =
      lambda_m +
      (2 * ecc - (1 / 4) * Math.pow(ecc, 3)) * Math.sin(lambda_m - omega) +
  ...
  let Fsw =
    (((So / Math.PI) * Math.pow(1 + ecc * Math.cos(lambda - omega), 2)) /
...
function plot_contour(input) {
  Plotly.newPlot(input.divId, input.data, input.layout, contour_plot_config);
}
...
function plot_contour_1(
  ...
  const round = (num, precision) => Number(Number(num).toFixed(precision));
...
const contour_2_year1 = document.getElementById("x_input_contour_2_kyear1"),
...
const contour_3_ecc_input = document.getElementById("x_input_contour_3_ecc"),
...
[contour_3_ecc_slide, contour_3_obl_slide, contour_3_long_slide].forEach(
  (e) => {
    e.onchange = () => {
      plot_contour_3(
...
[contour_3_ecc_input, contour_3_obl_input, contour_3_long_input].forEach(
  (e) => {
    e.onchange = () => {
      plot_contour_3(
        contour_3_ecc_input.value,
